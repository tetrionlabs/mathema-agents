...
description: Drive mathema's MCP server to orient on an unfamiliar codebase, propose claims, and get real verdicts, without reading more source than the job needs. Covers the fifteen tools, the reference resources and procedure prompts the server itself serves, the order that works, and the judgement calls the server cannot make for you. Use when a mathema MCP server is available and the task is "what can be proved about this code", "add claims", "why won't this derive", or any CDD loop step against a real repository.
---
